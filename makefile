...
MOUNTFLAGS := # -v $(CONFIG)/config/default.conf:/etc/netdata/netdata.conf
...
PORTFLAGS  := -p 19999:19999
PROXYFLAGS := --build-arg http_proxy=$(http_proxy) --build-arg https_proxy=$(https_proxy) --build-arg no_proxy=$(no_proxy)

RUNFLAGS   := -e PGID=$(PGID) -e PUID=$(PUID) --cap-add SYS_PTRACE -v /proc:/host/proc:ro -v /sys:/host/sys:ro
...
test :
	docker run --rm -it $(NAMEFLAGS) $(RUNFLAGS) $(PORTFLAGS) $(MOUNTFLAGS) $(OTHERFLAGS) $(IMAGETAG) sh -ec 'netdata --version'
